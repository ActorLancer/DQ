...
## 0. 阅读与归并说明（审阅补充，不删减正文）

为降低本文件在“需求基线 + 开发附录 + 专题设计补充”三层内容叠加后产生的阅读重复，本版在**不删减任何原始业务内容**的前提下，仅补充以下阅读归并规则：

### 0.1 三层结构

1. **需求基线层（1-30）**
   用于回答“做什么、为什么做、V1/V2/V3 边界是什么、核心对象和链路是什么”。

2. **开发附录层（31-39）**
   用于把基线需求落成研发前可冻结的模板、定价、赔付、字段、版本与开发调试规则。

3. **专题设计补充层（40-47）**
   用于把支付、认证、审计、一致性、搜索、推荐、可观测性、交易链监控等专题补齐为可落地设计约束。

### 0.2 重复主题的统一归属（阅读归并索引）

- **版本范围 / V1-V2-V3 边界**：以 **第 5 章** 为主基线；第 25、36、40-47 章中的版本边界视为专题补充，不单独改变主基线。
- **主体、身份、权限与作用域**：以 **第 9、17、41 章** 合并阅读；其中第 9 章定义对象，第 17 章定义权限粒度，第 41 章定义认证与会话。
- **产品类型、权利、SKU、模板**：以 **第 10、11、16、32-35 章** 合并阅读；其中第 11 章定义产品与权利，第 32-35 章定义模板、定价、赔付与字段。
- **支付、账单、结算、赔付**：以 **第 15.4、16.6、33、34、40 章** 合并阅读；其中第 40 章为支付专题主设计。
- **审计、证据、一致性、监控**：以 **第 23、39.10、42、43、46、47 章** 合并阅读；其中第 42-43 章定义审计与双层权威，第 46-47 章定义运行与交易链监控。
- **搜索、索引、推荐、发现**：以 **第 35、44、45 章** 合并阅读；其中第 35 章定义字段字典，第 44 章定义搜索与索引，第 45 章定义推荐。
- **链上链下边界 / 公链展示**：以 **第 8、16.8、21、43 章** 合并阅读；第 8 章给总原则，第 21/43 章给落地与一致性。

### 0.3 本版仅做的结构整理

- 修正“第 44 章”在文档中的出现顺序，使章节编号恢复单调递增。
- 将附录中的 `R3` 口径残留改为“原附录 R3 / 整合版 R4”的说明方式，避免版本误读。
- 将 V1 权利、SKU、模板映射统一收口到正文和第 48 章最终对照表。
- 删除已失效的历史解释和替换备注，仅保留当前生效口径。


### 0.4 当前统一口径（2026-04-11）

本版新增的裁决只解决已经出现的“口径漂移”问题，不扩张 V1 范围：

1. **V1 正式可售权利统一为 5 类**：使用权、访问权、查询权、结果获取权、有限内部共享权。
2. **“订阅”统一视为交付/计费形态**，不再单列为独立权利类型。
3. **“计算权”仍属于 V2**，V1 的沙箱/模板查询统一按“查询权 + 结果获取权”理解。
4. **V1 模板体系补齐到 5 类商品族、8 个标准 SKU**，其中新增并明确 `SHARE_RO` 与 `QRY_LITE`。

如正文存在重复描述，以后文保留的最终定义和第 48 章对照表为准。
...
- 业务字段：表达交易、授权、计费、交付、验收、结算事实，例如 `order_amount`、`billing_mode`、`delivery_mode`。
...
### 4）查询与受控执行产品
- 形式：查询沙箱、SQL 模板、TVF 模板、clean room lite、受控任务
- 适用：买方不拿原始明细，只执行被批准的查询或任务
- 要求：模板白名单、输出边界、导出限制、审计留痕
...
| 固定报告 / 结果产品 | V1 | 结果获取权、受限使用权 | 反向恢复原始明细、默认再分发、默认训练 | T-RESULT-STD | 一次性/按批次 | 结果包下载 / 报告交付 |
...
| `API_SUB` | API / 服务 | 访问权 | `Application + Key / OAuth` | `CONTRACT_API_SUB_V1 / LICENSE_API_SUB_V1 / DELIVERY_API_SUB_V1 / ACCEPT_API_SUB_V1 / REFUND_API_SUB_V1 / BILL_API_SUB_V1` |
| `API_PPU` | API / 服务 | 访问权 | `Application + Key / OAuth` | `CONTRACT_API_PPU_V1 / LICENSE_API_PPU_V1 / DELIVERY_API_PPU_V1 / ACCEPT_API_PPU_V1 / REFUND_API_PPU_V1 / BILL_API_PPU_V1` |
...
| `RPT_STD` | 固定报告 / 结果产品 | 结果获取权 + 受限使用权 | `报告交付 / 结果包下载` | `CONTRACT_REPORT_V1 / LICENSE_REPORT_V1 / DELIVERY_REPORT_V1 / ACCEPT_REPORT_V1 / REFUND_REPORT_V1 / BILL_REPORT_V1` |
...
> 本附录原文默认以 R3 为当前基线；在本整合版中按整份文档统一视为 R4 当前基线。若后续单独拆文档，附录内容可直接平移为独立规格说明。
...
- 统一按 **5 类商品族、8 个标准 SKU** 收口：`FILE_STD / FILE_SUB / SHARE_RO / API_SUB / API_PPU / QRY_LITE / SBX_STD / RPT_STD`。
- 每种 SKU 必须绑定一套默认模板：
  - 合同模板
  - 授权模板
  - 交付模板
...
| 文件包（更新订阅） | FILE_SUB | 周期文件更新 | 使用权 + 有限内部共享权（可选） | 月/季/年订阅 | 首次交付验收 + 周期履约 | 未到首交付日前可退款；交付后按剩余周期处理 |
...
| 报告/结果包 | RPT_STD | PDF/CSV/分析报告 | 结果获取权 + 受限使用权 + 有限内部共享权（可选） | 一次性收费 | 文件/结果回执验收 | 未交付可退款；交付后按内容不符争议处理 |
...
### 36.5 原附录 R3 版本的最终范围判断（在本整合版中按 R4 统一理解）

当前建议下，原附录的 R3 版本判断如下（在本整合版中按 R4 统一理解）：
...
如果你下一步继续推进到研发层，我建议以这份 R4 整合稿为主文档，再从中直接拆 4 份最小实现清单：
...
| 权利类型 | V1 是否正式支持 | 唯一解释 | 适用商品族 | 默认限制 | 必须绑定的控制点 |
|---|---|---|---|---|---|
...
| 模型训练权 | 否（V2） | 用于模型训练或微调 | V2 计算/模型产品 | V1 默认禁止 | 训练目的声明、样本边界、输出边界 |
...
3. `subscription` 统一不进入 rights_type，而进入 `billing_mode / delivery_mode / revision_subscription`。
4. V1 任意商品只要出现“算法提交、任务执行、算力上限、容器审核、证明材料齐全”等描述，必须标记为 V2 预留，不得默认落入 V1 沙箱商品。

### 48.2 商品族到标准 SKU 的唯一映射

| V1 商品族 | 标准 SKU | 是否 V1 正式支持 | 默认可售权利 | 默认交付方式 | 默认验收模板 | 默认退款模板 | 默认计费模板 | 备注 |
|---|---|---|---|---|---|---|---|---|
...
| API 订阅 | API_SUB | 是 | 访问权 + 结果获取权 | API Key / OAuth / 网关开通 | ACCEPT_API_V1 | REFUND_API_V1 | BILL_API_SUB_V1 | 周期费 + 调用量 |
...
| 固定报告/结果产品 | RPT_STD | 是 | 结果获取权 + 受限使用权 + 有限内部共享权（可选） | 报告交付 / 结果包下载 | ACCEPT_REPORT_V1 | REFUND_REPORT_V1 | BILL_REPORT_V1 | 对应固定报告、结果包 |

### 48.3 各商品族的唯一主模板族

| 商品族 | 合同模板 | 授权模板 | 交付模板 | 验收模板 | 退款模板 | 赔付模板 | 计费模板 |
|---|---|---|---|---|---|---|---|
...
| API 订阅 | CONTRACT_API_V1 | LICENSE_API_ACCESS_V1 | DELIVERY_API_V1 | ACCEPT_API_V1 | REFUND_API_V1 | COMP_API_V1 | BILL_API_SUB_V1 |
| API 按量 | CONTRACT_API_PPU_V1 | LICENSE_API_ACCESS_V1 | DELIVERY_API_PPU_V1 | ACCEPT_API_PPU_V1 | REFUND_API_PPU_V1 | COMP_API_PPU_V1 | BILL_API_PPU_V1 |
...
- **五类商品族与六种 V1 交付形态**：文件快照/版本订阅、只读共享、API/服务、模板查询 lite、查询沙箱、固定报告/结果产品
...
## 49. 工程约束冻结补遗（2026-04-11）

本章用于补齐“已经有产品方向，但还没冻结到足够让研发/测试/运维/法务同解”的工程约束。若与前文旧表述冲突，以本章为准。
