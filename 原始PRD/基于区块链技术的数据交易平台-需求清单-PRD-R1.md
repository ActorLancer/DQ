...
| 产品类型 | 文件快照/版本订阅、只读共享、API/订阅、查询沙箱/模板查询 lite、固定报告/结果产品 | 组合包销售、卖方自持共享增强 | C2D、FL、MPC、TEE、ZKP 正式交付 |
...
| 查询沙箱 / 模板查询 lite / 报告服务 | V1 | 查询权、结果获取权 | 原始明细导出、脚本逃逸、结果逆向识别 | T-SBX-STD | 席位/月费/按次 | 沙箱账号 / 模板查询 / 报告交付 |
| 结果产品 / 合成数据 / 特征结果 | V1 | 结果获取权、受限使用权 | 反向恢复原始明细、默认再分发、默认训练 | T-RESULT-STD | 一次性/按批次 | 结果包下载 / 报告交付 |
...
- `V1` 正式商品化范围：文件快照/版本订阅、只读共享、API/服务、查询沙箱/模板查询 lite、固定报告/结果产品
...
4. V1 只允许上架文件快照/版本订阅、只读共享、API/服务、查询沙箱/模板查询 lite、固定报告/结果产品类 SKU。
...
- 文件快照/版本订阅、只读共享、API/服务、查询沙箱/模板查询 lite、固定报告/结果产品五类 SKU
- 自动规则初审 + 人工复核
- 数字合约、电子签章、联盟链存证
...
| 交付方式 | delivery_mode | Enum | 是 | 是 | 是 | 文件快照/共享/API/模板查询/结果包 |
