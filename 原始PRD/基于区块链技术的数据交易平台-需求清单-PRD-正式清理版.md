...
3. **Department（部门）**：组织内部的权限与成本中心，如采购部、租户管理员（法务权限包）部、数据治理部。
...
- 平台内的跨链治理、生态合作、连接器接入、可观测性治理、监管协同等专题职责，统一挂接到上述核心角色的权限包上，不再继续新增一级岗位型角色。
- 租户侧的法务、财务、数据治理、开发调试等职责，通过租户管理员、卖方运营员、买方运营员、租户开发者及其权限包表达。
...
| 对象 | 典型角色 | 可执行动作 | 默认禁止 |
|---|---|---|---|
| 租户管理员 | 租户管理员 | 用户/部门/应用管理、账单查看、审批发起 | 越过法务/合规权限包直接放行高风险交易 |
| Department 审批人 | 部门负责人 | 部门级订单审批、预算确认 | 修改合同模板 |
| User-采购角色 | 买方运营员 | RFQ、下单、验收、续费 | 创建供方产品 |
| User-法务角色 | 租户管理员（法务权限包） | 合同审阅、条款审批 | 修改技术策略 |
| User-数据治理角色 | 卖方运营员 | 产品上架、元数据维护、样例上传 | 直接确认付款 |
| Application | 系统账号 | API 调用、批量查询 | 登录控制台、转授权 |
| Connector 运维 | 平台管理员（生态合作权限包） | 健康检查、策略下发 | 查看订单财务信息 |
| 监管/合作方只读身份 | 监管节点/监管账号 | 查看监管视图、导出证据包 | 介入业务配置 |
...
|---|---|---|---|---|
| R-001 | 短时高频下载/超限调用 | 限流 + 告警 | 平台风控结算员 | 30 分钟 |
...
| R-006 | 已签约产品被判定违规 | 冻结新购/续费，评估存量访问 | 合规 + 租户管理员（法务权限包） | 4 小时 |
...
- 对重要场景应支持生成面向监管、租户管理员（法务权限包）、客户的结构化证明材料
...
| 结果级商业输出权 | 否（V2） | 将结果用于对外商业服务 | V2 模型/结果产品 | V1 默认禁止 | 租户管理员（法务权限包）审批、合同附加条款 |
...
| 合同生效审批 | 非标准条款、大额订单、特殊用途 | 租户管理员（法务权限包） + 业务负责人 | 放行/驳回 |
