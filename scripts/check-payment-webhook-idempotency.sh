...
log "running TEST-011 payment webhook idempotency checker with env=${ENV_FILE}"

log "ensuring local environment baseline"
ENV_FILE="${ENV_FILE}" bash ./scripts/smoke-local.sh

log "verifying mock payment provider baseline"
./scripts/check-mock-payment.sh
ok "mock payment baseline passed"

log "verifying webhook duplicate / success->fail / timeout->success protection"
TRADE_DB_SMOKE=1 \
cargo test -p platform-core bil005_payment_webhook_db_smoke -- --nocapture
ok "payment webhook smoke passed"

ok "TEST-011 payment webhook idempotency checker passed"
