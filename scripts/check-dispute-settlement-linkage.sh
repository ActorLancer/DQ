...
log "running TEST-013 dispute-settlement linkage checker with env=${ENV_FILE}"

log "ensuring local environment baseline"
ENV_FILE="${ENV_FILE}" bash ./scripts/smoke-local.sh

log "verifying dispute freeze and settlement recompute linkage"
TRADE_DB_SMOKE=1 \
cargo test -p platform-core bil019_dispute_refund_compensation_recompute_db_smoke -- --nocapture
ok "dispute settlement linkage smoke passed"

ok "TEST-013 dispute settlement linkage checker passed"
