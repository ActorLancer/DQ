...
- `check-performance-smoke.sh`：`TEST-018` 正式性能冒烟 checker；会复用 `smoke-local.sh`、`seed-local-iam-test-identities.sh`、`seed-demo.sh` 与 Keycloak password grant，真实执行搜索、下单、合同确认、锁资、交付、审计联查链路，校验四条正式 API 单次 `<= 2.0s`，并落盘 Prometheus / `/metrics` / request log artifact。
- `check-failure-drills.sh`：`TEST-019` 正式故障演练 checker；会复用 `smoke-local.sh`、`seed-local-iam-test-identities.sh`、`seed-demo.sh` 与 `check-mock-payment.sh`，真实执行 Kafka 停机、OpenSearch 不可用、Fabric Adapter 停机、Mock Payment 延迟四类故障注入，并回查 `health/deps`、`trade.order_main / ops.outbox_event`、Kafka consumer lag、`ops.external_fact_receipt` 与 live mock payment timeout 输出。
- `check-rollback-recovery.sh`：`TEST-020` 正式回滚恢复 checker；会复用 `smoke-local.sh`、`down-local.sh`、`up-local.sh`、`db/scripts/migrate-reset.sh`、`seed-local-iam-test-identities.sh`、`seed-demo.sh` 与 `check-demo-seed.sh`，验证业务库 reset 后的基础 seed replay、正式运行态恢复与 demo 数据恢复。
- `check-standard-sku-coverage.sh`：`TEST-023` 正式 SKU 覆盖矩阵 checker；会复用 `smoke-local.sh`、`check-demo-fixtures.sh`、`seed-demo.sh`、`trade008~015`、`bil017/018/019/024/025/026` 与 `cat023_standard_scenarios_endpoint_db_smoke`，并通过 `check-standard-sku-coverage.mjs` 真实回查 `catalog.standard-scenarios` 与 demo 订单 `billing` 读模型，输出 `8 SKU × 5 场景` matrix artifact。
...
- `check-notification-smoke.sh`：`TEST-027` 正式通知 smoke checker；会复用 `smoke-local.sh`，拉起宿主机 `outbox-publisher + notification-worker`，执行 `notif004 / notif005 / notif006 / notif007` 四类业务事件通知、`platform-core /api/v1/ops/notifications/audit/search` 正式联查，以及隔离的 `notif012_notification_worker_live_smoke`，并通过 `check-notification-smoke.mjs` 汇总 raw artifact 和 summary。
- `check-canonical-contracts.sh`：`TEST-028` 正式 canonical checker；本地默认 `full` 模式会串行执行 `check-openapi-schema.sh + check-topic-topology.sh + smoke-local.sh`，CI 使用 `CANONICAL_CHECK_MODE=static` 跑静态子集；两种模式都会写入 `target/test-artifacts/canonical-contracts/summary.json`，并显式拦截宿主机 Kafka 边界误用、旧 topic 默认值、第二份本地 topic manifest，以及把 `check-topic-topology.sh` 误报成全量 topic smoke 的文档漂移。
- `check-api-contract-baseline.sh`：`TEST-003` 正式 contract checker；校验 OpenAPI 成功/失败 envelope、关键响应字段、错误码基线、订单状态机 action enum / 禁止错误码绑定，并新增运行时代码口径静态校验（拒绝 `message` 业务前缀与 `code` 不一致、拒绝 helper 继续发通用 fallback code）。它不替代 `TEST-028` 的 canonical smoke。
