...
log "running TEST-014 audit replay dry-run checker with env=${ENV_FILE}"

log "verifying replay route permission and dry-run guards"
cargo test -p platform-core rejects_replay_job_without_permission -- --nocapture
cargo test -p platform-core replay_job_requires_step_up -- --nocapture
cargo test -p platform-core replay_job_enforces_dry_run_only -- --nocapture
cargo test -p platform-core rejects_replay_lookup_without_permission -- --nocapture
ok "replay route guards passed"

log "ensuring local environment baseline"
ENV_FILE="${ENV_FILE}" bash ./scripts/smoke-local.sh

log "verifying audit replay dry-run live smoke"
AUD_DB_SMOKE=1 \
cargo test -p platform-core audit_trace_api_db_smoke -- --nocapture
ok "audit replay dry-run live smoke passed"

ok "TEST-014 audit replay dry-run checker passed"
