...
`SEARCHREC-016` 之后，`search.yaml` 与 `recommendation.yaml` 的最小要求固定为：

- `packages/openapi/{search,recommendation}.yaml` 与 `docs/02-openapi/{search,recommendation}.yaml` 必须逐字同步
- `./scripts/check-openapi-schema.sh` 必须覆盖 Bearer 鉴权、正式权限点、`X-Idempotency-Key`、必要 `X-Step-Up-Token`、审计/访问日志副作用与 `x-role` 清理
- README 索引必须明确说明这两份文件已经进入正式归档与实现对齐校验
...
- `search.yaml`：`AUD-022 + SEARCHREC-016` 已同步归档并校验 Search/Ops Search 的正式契约，覆盖 Bearer 鉴权、正式权限点、`X-Idempotency-Key`、必要 `X-Step-Up-Token`、审计/访问日志副作用、`SEARCH_*` 错误码与 request/response schema。
- `recommendation.yaml`：`SEARCHREC-016` 已同步归档并校验 Recommendation/Ops Recommendation 的正式契约，覆盖 Bearer 鉴权、正式权限点、`X-Idempotency-Key`、必要 `X-Step-Up-Token`、审计/访问日志副作用与 request/response schema。
