...
- 当前活动分卷以入口页为准
- 本卷保留早期模板、示例与历史批次记录，后续不作为流程文档默认入口
...
- 状态：通过
- 当前任务编号：TRADE-002
- 当前批次目标：实现价格快照模型，在订单创建阶段可固化 SKU 价格、计费模式、结算口径、退款口径与税务信息快照，并提供可联调验证入口。
- 前置依赖核对结果：`CORE-014; DB-006; IAM-001; CAT-001` 已完成并通过审批；`BATCH-110` 已通过，可继续执行。
- 已阅读证据（文件+要点）：
  1. `docs/开发任务/v1-core-开发任务清单.csv`：定位 `TRADE-002` 交付、DoD、acceptance 与 `technical_reference`。
  2. `docs/开发任务/v1-core-开发任务清单.md`：核对阅读版任务解释与 CSV 一致。
  3. `docs/开发任务/Agent-开发与半人工审核流程.md`：遵循“计划中->编码->验证->待审批”。
  4. `docs/开发任务/AI-Agent-执行提示词.md`：保持 V1 范围，不抢跑后续功能。
  5. `docs/开发任务/V1-Core-实施进度日志.md`：已记录计划中并追加待审批。
  6. `docs/开发任务/V1-Core-TODO与预留清单.md`：已同步批次记录。
  7. `docs/开发任务/V1-Core-人工审批记录.md`：仅读规则；继续由人工维护。
  8. `docs/全集成文档/数据交易平台-全集成基线-V1.md`：核对交易闭环步骤与订单前置/下单约束。
  9. `docs/开发准备/服务清单与服务边界正式版.md`：确认 trade/order/contract/authorization 边界。
  10. `docs/开发准备/接口清单与OpenAPI-Schema冻结表.md`：确认 `/api/v1` 接口风格与返回结构约束。
  11. `docs/开发准备/事件模型与Topic清单正式版.md`：确认事件非主状态、审计留痕要求。
  12. `docs/开发准备/统一错误码字典正式版.md`：沿用 `TRD_STATE_CONFLICT` / `IAM_UNAUTHORIZED`。
  13. `docs/开发准备/测试用例矩阵正式版.md`：执行单测+联调+DB证据验证。
  14. `docs/开发准备/仓库拆分与目录结构建议.md`：按功能拆分 `order` 模块文件。
  15. `docs/开发准备/本地开发环境与中间件部署清单.md`：使用 `datab-postgres:5432` 做 API 联调。
  16. `docs/开发准备/配置项与密钥管理清单.md`：按 `DATABASE_URL/KAFKA_BROKERS/...` 启动。
  17. `docs/开发准备/技术选型正式版.md`：遵循 PostgreSQL 主状态权威原则。
  18. `docs/开发准备/平台总体架构设计草案.md`：保持模块化单体与生命周期对象口径。
- technical_reference 约束映射：
  1. `docs/领域模型/全量领域模型与对象关系说明.md:L620`：在 `Order` 聚合新增可复用价格快照对象 `OrderPriceSnapshot`。
  2. `docs/全集成文档/数据交易平台-全集成基线-V1.md:L1723`：订单创建阶段的价格/结算/退款/税务要素可冻结并持久化。
  3. `docs/业务流程/业务流程图-V1-完整版.md:L204`：搜索选购后进入下单前，价格与交易口径需可固化追溯。
- 已实现功能：
  1. 新增领域模型 `OrderPriceSnapshot`（含 `pricing_mode/unit_price/currency_code/billing_mode/refund_mode/settlement_terms/tax_terms`）。
  2. 新增结算口径派生规则 `derive_settlement_basis`。
  3. 新增仓储逻辑：从 `catalog.product + catalog.product_sku` 读取价格与 SKU 交易属性，固化到 `trade.order_main.price_snapshot_json`，并同步 `fee_preview_snapshot`。
  4. 新增 API：`POST /api/v1/trade/orders/{id}/price-snapshot/freeze`。
  5. 新增审计动作：`trade.order.price_snapshot.freeze`。
  6. 更新 OpenAPI：`packages/openapi/trade.yaml` 增补路径与快照 schema。
  7. 新增测试：权限测试、快照口径单测、`TRADE_DB_SMOKE` 数据库烟测。
- 涉及文件：
  - `apps/platform-core/src/modules/order/api/handlers.rs`
  - `apps/platform-core/src/modules/order/api/mod.rs`
  - `apps/platform-core/src/modules/order/domain/mod.rs`
  ...
  - `apps/platform-core/src/modules/order/dto/mod.rs`
  - `apps/platform-core/src/modules/order/dto/price_snapshot.rs`
  - `apps/platform-core/src/modules/order/repo/mod.rs`
  - `apps/platform-core/src/modules/order/repo/pre_request_repository.rs`
  - `apps/platform-core/src/modules/order/repo/price_snapshot_repository.rs`
  - `apps/platform-core/src/modules/order/tests/mod.rs`
  - `apps/platform-core/src/modules/order/tests/trade002_price_snapshot_db.rs`
  - `packages/openapi/trade.yaml`
  - `docs/开发任务/V1-Core-实施进度日志.md`
  - `docs/开发任务/V1-Core-TODO与预留清单.md`
- 验证步骤：
  ...
  3. 启动服务：
     `APP_PORT=18080 APP_HOST=127.0.0.1 DATABASE_URL=postgres://datab:datab_local_pass@127.0.0.1:5432/datab KAFKA_BROKERS=127.0.0.1:9094 KAFKA_BOOTSTRAP_SERVERS=127.0.0.1:9094 MINIO_ENDPOINT=http://127.0.0.1:9000 OPENSEARCH_ENDPOINT=http://127.0.0.1:9200 cargo run -p platform-core`
  4. 准备联调测试订单（按你允许策略插入测试数据）。
  5. `curl -X POST /api/v1/trade/orders/{id}/price-snapshot/freeze`
  6. `psql` 校验 `price_snapshot_json/fee_preview_snapshot/audit.audit_event`。
- 验证结果：
  - `cargo test -p platform-core`：通过（`88 passed, 0 failed, 1 ignored`）。
  - API 联调通过：
    - `request_id=req-trade002-1776515593`
    - `order_id=31111111-1111-1111-1111-111111111111`
    - 返回快照字段：`pricing_mode=one_time`、`billing_mode=one_time`、`refund_mode=manual_refund`、`settlement_basis=one_time_final`、`tax_policy=platform_default`。
  - DB 证据：
    - `trade.order_main.price_snapshot_json` 已写入对应字段。
    - `trade.order_main.fee_preview_snapshot.pricing_mode=one_time`。
    - `audit.audit_event`（`action_name=trade.order.price_snapshot.freeze`）计数=`1`。
  - 说明：联调库初始 `trade.order_main` 无数据，已按流程插入最小测试订单行后完成验证。
- 覆盖的冻结文档条目：
  - `docs/领域模型/全量领域模型与对象关系说明.md`（4.4 交易与订单聚合）
  - `docs/全集成文档/数据交易平台-全集成基线-V1.md`（15 核心交易链路）
  - `docs/业务流程/业务流程图-V1-完整版.md`（4.3 买方搜索、选购与下单流程）
  - `docs/开发准备/接口清单与OpenAPI-Schema冻结表.md`（Trade 接口风格）
- 覆盖的任务清单条目：`TRADE-002`
- 未覆盖项：无。
- 新增 TODO / 预留项：无新增 `TODO(V1-gap)` / `TODO(V2-reserved)` / `TODO(V3-reserved)`；`TODO-PROC-BIL-001` 追溯约束保持不变。
- 待人工审批结论：通过
...
- 状态：通过
- 当前任务编号：TRADE-003
- 当前批次目标：实现 `POST /api/v1/orders`，完成商品状态校验、价格快照固化、权限校验、风险阻断与审计留痕，并完成 API 联调与数据库证据验证。
- 前置依赖核对结果：`CORE-014; DB-006; IAM-001; CAT-001` 均已完成且通过审批；上一批 `BATCH-111` 已通过，可继续执行。
- 已阅读证据（文件+要点）：
  1. `docs/开发任务/v1-core-开发任务清单.csv`：定位 `TRADE-003` 的描述、DoD、验收项、依赖和 `technical_reference`。
  2. `docs/开发任务/v1-core-开发任务清单.md`：核对阅读版任务解释，确认以 CSV 为准。
  3. `docs/开发任务/Agent-开发与半人工审核流程.md`：遵循“计划中 -> 编码 -> 验证 -> 待审批”。
  4. `docs/开发任务/AI-Agent-执行提示词.md`：保持 V1 边界，不引入 V2/V3 正式能力。
  5. `docs/开发任务/V1-Core-实施进度日志.md`：先登记计划中，再补全待审批结果。
  6. `docs/开发任务/V1-Core-TODO与预留清单.md`：同步批次记录并保持 `TODO-PROC-BIL-001` 追溯要求。
  7. `docs/开发任务/V1-Core-人工审批记录.md`：仅阅读规则，继续由人工维护。
  8. `docs/全集成文档/数据交易平台-全集成基线-V1.md`：核对核心交易链路“下单/锁资/交付前置”主轴约束。
  9. `docs/开发准备/服务清单与服务边界正式版.md`：确认 `order` 负责主状态与审计一致性。
  10. `docs/开发准备/接口清单与OpenAPI-Schema冻结表.md`：确认 `POST /api/v1/orders` 为 V1 冻结接口。
  11. `docs/开发准备/事件模型与Topic清单正式版.md`：确认事件非主状态权威，主状态落 PostgreSQL。
  12. `docs/开发准备/统一错误码字典正式版.md`：对齐 `ORDER_CREATE_FORBIDDEN` 语义与现有码体系映射。
  13. `docs/开发准备/测试用例矩阵正式版.md`：执行“权限+状态+审计+联调”验证策略。
  14. `docs/开发准备/仓库拆分与目录结构建议.md`：按功能拆分 `order` DTO/Repo/Test 文件。
  15. `docs/开发准备/本地开发环境与中间件部署清单.md`：使用 `datab-postgres:5432` 联调验证。
  16. `docs/开发准备/配置项与密钥管理清单.md`：按环境变量口径启动并联调。
  17. `docs/开发准备/技术选型正式版.md`：遵循 PostgreSQL 主状态机权威。
  18. `docs/开发准备/平台总体架构设计草案.md`：保持模块化单体边界与生命周期对象一致。
- technical_reference 约束映射：
  1. `docs/领域模型/全量领域模型与对象关系说明.md:L620`：订单主对象需包含 `product/sku/buyer/seller/current_state/payment_status/price_snapshot` 等核心字段。
  2. `docs/全集成文档/数据交易平台-全集成基线-V1.md:L1723`：下单属于核心交易闭环主链，需在合同/支付前完成主状态与审计留痕。
  3. `docs/业务流程/业务流程图-V1-完整版.md:L204`：下单时必须校验“商品可售 + 风险阻断 + 价格快照固化”。
- 已实现功能：
  1. 新增接口 `POST /api/v1/orders`，返回订单创建结果与价格快照。
  2. 新增订单创建 DTO：`CreateOrderRequest/CreateOrderResponse/CreateOrderResponseData`。
  3. 新增仓储实现 `order_create_repository`：完成商品与 SKU 校验、组织状态校验、风险阻断、订单入库、价格快照固化。
  4. 订单创建落库采用单事务：`trade.order_main` 写入与 `audit.audit_event(trade.order.create)` 同事务提交。
  5. 支持 `x-idempotency-key` 幂等重放查询，并记录 `trade.order.create.idempotent_replay` 审计。
  6. 新增测试：权限拒绝用例 + `TRADE_DB_SMOKE` 数据库烟测 `trade003_create_order_db_smoke`。
  7. 更新 OpenAPI：`packages/openapi/trade.yaml` 增补 `POST /api/v1/orders` 与对应 schema。
- 涉及文件：
  - `apps/platform-core/src/modules/order/api/handlers.rs`
  - `apps/platform-core/src/modules/order/api/mod.rs`
  - `apps/platform-core/src/modules/order/dto/mod.rs`
...
- 验证步骤：
  ...
  5. `TRADE_DB_SMOKE=1 DATABASE_URL=postgres://datab:datab_local_pass@127.0.0.1:5432/datab cargo test -p platform-core trade003_create_order_db_smoke -- --nocapture`
  6. 启动服务并联调：
     `APP_PORT=18080 APP_HOST=127.0.0.1 DATABASE_URL=postgres://datab:datab_local_pass@127.0.0.1:5432/datab KAFKA_BROKERS=127.0.0.1:9094 KAFKA_BOOTSTRAP_SERVERS=127.0.0.1:9094 MINIO_ENDPOINT=http://127.0.0.1:9000 OPENSEARCH_ENDPOINT=http://127.0.0.1:9200 cargo run -p platform-core`
  7. `curl -X POST http://127.0.0.1:18080/api/v1/orders`（使用临时 seed 数据）。
  8. `psql` 校验 `trade.order_main` 与 `audit.audit_event`。
- 验证结果：
  - `cargo test -p platform-core`：通过（`90 passed, 0 failed, 1 ignored`）。
  - `TRADE_DB_SMOKE` 专项测试：通过（`trade003_create_order_db_smoke ... ok`）。
  - `curl` 联调通过，证据如下：
    - `request_id=req-trade003-api-1776517577`
    - `order_id=7e4d9223-f58d-49ec-8423-90381bf2606c`
    - `response.status=created`
    - `response.payment_status=unpaid`
    - `response.price_snapshot.billing_mode=one_time`
  - DB 校验结果：
    - `trade.order_main`：`status=created|payment_status=unpaid|price_snapshot_json.billing_mode=one_time`
    - `audit.audit_event`：`action_name=trade.order.create` 计数 `AUDIT_COUNT=1`
  - 测试数据已执行清理（临时 `organization/data_asset/asset_version/product/sku/order` 已删除）。
- 覆盖的冻结文档条目：
  - `docs/领域模型/全量领域模型与对象关系说明.md`（4.4 交易与订单聚合）
  - `docs/全集成文档/数据交易平台-全集成基线-V1.md`（15 核心交易链路设计）
  - `docs/业务流程/业务流程图-V1-完整版.md`（4.3 买方搜索、选购与下单流程）
  - `docs/开发准备/接口清单与OpenAPI-Schema冻结表.md`（5.6 订单接口）
  - `docs/权限设计/接口权限校验清单.md`（3.4 `POST /api/v1/orders` 校验项）
- 覆盖的任务清单条目：`TRADE-003`
- 未覆盖项：无。
- 新增 TODO / 预留项：无新增 `TODO(V1-gap)` / `TODO(V2-reserved)` / `TODO(V3-reserved)`；`TODO-PROC-BIL-001` 追溯约束保持不变。
- 待人工审批结论：通过
...
- 状态：通过
- 当前任务编号：TRADE-004
- 当前批次目标：实现 `GET /api/v1/orders/{id}`，返回订单主状态与分层子状态（payment/delivery/acceptance/settlement/dispute），并完成权限、审计与联调验证。
- 前置依赖核对结果：`CORE-014; DB-006; IAM-001; CAT-001` 已完成且审批通过；`BATCH-112` 已通过，可继续执行。
- 已阅读证据（文件+要点）：
  1. `docs/开发任务/v1-core-开发任务清单.csv`：定位 `TRADE-004` 交付、DoD、验收与 `technical_reference`。
  2. `docs/开发任务/v1-core-开发任务清单.md`：核对阅读版定义与 CSV 一致。
  3. `docs/开发任务/Agent-开发与半人工审核流程.md`：执行“计划中→编码→验证→待审批”。
  4. `docs/开发任务/AI-Agent-执行提示词.md`：保持 V1 范围与冻结边界。
  5. `docs/开发任务/V1-Core-实施进度日志.md`：先登记计划中，再追加待审批。
  6. `docs/开发任务/V1-Core-TODO与预留清单.md`：同步批次记录并保持 `TODO-PROC-BIL-001` 追溯。
  7. `docs/开发任务/V1-Core-人工审批记录.md`：按规则仅读，由人工维护。
  8. `docs/全集成文档/数据交易平台-全集成基线-V1.md`：核对核心交易链路中订单详情与状态展示语义。
  9. `docs/开发准备/服务清单与服务边界正式版.md`：确认交易主状态归属 `platform-core + PostgreSQL`。
  10. `docs/开发准备/接口清单与OpenAPI-Schema冻结表.md`：对齐 `GET /api/v1/orders/{id}` 接口冻结口径。
  11. `docs/开发准备/事件模型与Topic清单正式版.md`：确认事件非主状态机。
  12. `docs/开发准备/统一错误码字典正式版.md`：沿用现有码体系映射。
  13. `docs/开发准备/测试用例矩阵正式版.md`：覆盖主状态与子状态读取、审计留痕。
  14. `docs/开发准备/仓库拆分与目录结构建议.md`：按功能拆分 DTO/Repo/Test 文件。
  15. `docs/开发准备/本地开发环境与中间件部署清单.md`：使用 `datab-postgres:5432` 做 API 联调。
  16. `docs/开发准备/配置项与密钥管理清单.md`：按环境变量口径运行联调。
  17. `docs/开发准备/技术选型正式版.md`：坚持 PostgreSQL 主状态权威。
  18. `docs/开发准备/平台总体架构设计草案.md`：保持模块化单体和状态边界。
- technical_reference 约束映射：
  1. `docs/领域模型/全量领域模型与对象关系说明.md:L620`：订单详情需体现主状态与关键子状态字段。
  2. `docs/全集成文档/数据交易平台-全集成基线-V1.md:L1723`：订单对象在闭环链路中承担进度可见性与可追溯责任。
  3. `docs/业务流程/业务流程图-V1-完整版.md:L204`：下单后应可查询主状态与支付/交付/验收分层进度。
- 已实现功能：
  1. 新增接口 `GET /api/v1/orders/{id}`。
  2. 新增订单详情 DTO（`GetOrderDetailResponse/GetOrderDetailResponseData`）。
  3. 新增仓储查询 `load_order_detail`，返回 `current_state + payment_status + delivery_status + acceptance_status + settlement_status + dispute_status`。
  4. 新增分层子状态推导逻辑（基于 `status/payment_status` 映射默认分层状态）。
  5. 新增权限矩阵（`trade.order.read` 角色集合）与租户作用域校验（买方/卖方任一组织可读）。
  6. 新增审计动作 `trade.order.read`。
  7. 更新 OpenAPI：补充 `GET /api/v1/orders/{id}` 与订单详情 schema。
  8. 新增测试：权限拒绝用例 + `trade004_order_detail_db_smoke` 数据库烟测。
- 涉及文件：
  - `apps/platform-core/src/modules/order/api/handlers.rs`
  - `apps/platform-core/src/modules/order/api/mod.rs`
  - `apps/platform-core/src/modules/order/dto/mod.rs`
...
- 验证步骤：
  ...
  2. `cargo test -p platform-core`
  3. `ENV_FILE=infra/docker/.env.local ./scripts/check-local-stack.sh core`
  4. `TRADE_DB_SMOKE=1 DATABASE_URL=postgres://datab:datab_local_pass@127.0.0.1:5432/datab cargo test -p platform-core trade004_order_detail_db_smoke -- --nocapture`
  5. 启动服务并联调：
     `APP_PORT=18080 APP_HOST=127.0.0.1 DATABASE_URL=postgres://datab:datab_local_pass@127.0.0.1:5432/datab KAFKA_BROKERS=127.0.0.1:9094 KAFKA_BOOTSTRAP_SERVERS=127.0.0.1:9094 MINIO_ENDPOINT=http://127.0.0.1:9000 OPENSEARCH_ENDPOINT=http://127.0.0.1:9200 cargo run -p platform-core`
  6. `curl -X GET http://127.0.0.1:18080/api/v1/orders/{id}`（临时 seed 订单）。
  7. `psql` 校验 `trade.order_main` 主状态与 `audit.audit_event` 留痕。
- 验证结果：
  - `cargo test -p platform-core` 通过（`92 passed, 0 failed, 1 ignored`）。
  - `trade004_order_detail_db_smoke` 通过（`1 passed, 0 failed`）。
  - API 联调通过，证据如下：
    - `request_id=req-trade004-api-1776519176`
    - `order_id=74ded5b4-6624-4185-9460-10185afbd14d`
    - 返回状态：`current_state=created`、`payment_status=unpaid`、`delivery_status=pending_delivery`、`acceptance_status=not_started`、`settlement_status=not_started`、`dispute_status=none`
  - DB 证据：
    - `trade.order_main`：`status=created|payment_status=unpaid`
    - `audit.audit_event`：`action_name=trade.order.read` 计数 `AUDIT_COUNT=1`
  - 测试数据已清理（临时 seed 的 order/product/asset/org 均已删除）。
- 覆盖的冻结文档条目：
  - `docs/领域模型/全量领域模型与对象关系说明.md`（4.4 交易与订单聚合）
  - `docs/全集成文档/数据交易平台-全集成基线-V1.md`（15 核心交易链路）
  - `docs/业务流程/业务流程图-V1-完整版.md`（4.3 买方搜索、选购与下单流程）
...
- 状态：计划中
- 当前任务编号：TRADE-006
- 当前批次目标：实现 `POST /api/v1/orders/{id}/contract-confirm`，关联数字合约快照与签署角色，补齐权限、审计与 OpenAPI。
- 前置依赖核对结果：`CORE-014; DB-006; IAM-001; CAT-001` 已完成且审批通过；`BATCH-114` 已通过，可继续执行。
- 计划验证步骤：
  1. `cargo fmt --all`
  2. `cargo test -p platform-core`
  3. `ENV_FILE=infra/docker/.env.local ./scripts/check-local-stack.sh core`
  4. `TRADE_DB_SMOKE=1 DATABASE_URL=postgres://datab:datab_local_pass@127.0.0.1:5432/datab cargo test -p platform-core trade006_contract_confirm_db_smoke -- --nocapture`
...
- 状态：待审批
- 当前任务编号：TRADE-006
- 当前批次目标：实现 `POST /api/v1/orders/{id}/contract-confirm`，关联数字合约快照与签署角色。
- 前置依赖核对结果：`CORE-014; DB-006; IAM-001; CAT-001` 已完成且审批通过；`BATCH-114` 已通过。
- 已阅读证据（文件+要点）：
  1. `docs/开发任务/v1-core-开发任务清单.csv`：确认 `TRADE-006` 交付件、DoD、验收与 technical_reference。
  2. `docs/开发任务/v1-core-开发任务清单.md`：核对任务解释与约束一致。
  3. `docs/开发任务/Agent-开发与半人工审核流程.md`：遵循计划中→编码→验证→待审批。
  4. `docs/开发任务/AI-Agent-执行提示词.md`：执行源与冻结边界。
  5. `docs/开发任务/V1-Core-实施进度日志.md`：记录本批计划与结果。
  6. `docs/开发任务/V1-Core-TODO与预留清单.md`：同步 TODO 追溯记录。
  7. `docs/开发任务/V1-Core-人工审批记录.md`：只读，按规则不写。
  8. `docs/全集成文档/数据交易平台-全集成基线-V1.md`：合同与授权位于核心链路步骤 5。
  9. `docs/开发准备/服务清单与服务边界正式版.md`：交易编排与合同落库边界。
  10. `docs/开发准备/接口清单与OpenAPI-Schema冻结表.md`：Trade 接口冻结口径对齐。
  11. `docs/开发准备/事件模型与Topic清单正式版.md`：主状态机与审计职责边界。
  12. `docs/开发准备/统一错误码字典正式版.md`：沿用 `TRD_STATE_CONFLICT` / `IAM_UNAUTHORIZED`。
  13. `docs/开发准备/测试用例矩阵正式版.md`：覆盖合同确认正反路径与审计留痕。
  14. `docs/开发准备/仓库拆分与目录结构建议.md`：按 DTO/Repo/Test 分拆实现。
  15. `docs/开发准备/本地开发环境与中间件部署清单.md`：按 `datab-postgres:5432` 联调。
  16. `docs/开发准备/配置项与密钥管理清单.md`：使用 `DATABASE_URL/KAFKA_BROKERS` 运行口径。
  17. `docs/开发准备/技术选型正式版.md`：PostgreSQL 为状态权威。
  18. `docs/开发准备/平台总体架构设计草案.md`：保持模块化边界。
- technical_reference 约束映射：
  1. `docs/领域模型/全量领域模型与对象关系说明.md:L530`：`DigitalContract` 需承接合同快照与签署结果。
  2. `docs/页面说明书/页面说明书-V1-完整版.md:L522`：合同确认页需呈现合同摘要并执行确认签署动作。
  3. `docs/全集成文档/数据交易平台-全集成基线-V1.md:L1723`：合同确认处于核心交易链路步骤 5，并需可审计。
- 已实现功能：
  1. 新增接口 `POST /api/v1/orders/{id}/contract-confirm`。
  2. 新增请求/响应 DTO：`ConfirmOrderContractRequest/Response`。
  3. 新增仓储事务：锁定订单、校验可确认状态、写入/更新 `contract.digital_contract` 合同快照、写入 `contract.contract_signer` 签署角色、回填 `trade.order_main.contract_id` 并推进状态到 `contract_effective`。
  4. 新增校验：权限 `trade.contract.confirm`、租户作用域、`x-user-id` 必填、签署角色白名单。
  5. 新增审计动作：`trade.contract.confirm`。
  6. 新增最小回归测试：权限拒绝 + DB smoke（成功/冲突分支）。
  7. 更新 OpenAPI：新增 `contract-confirm` path 与 schema。
- 涉及文件：
  - `apps/platform-core/src/modules/order/api/handlers.rs`
  - `apps/platform-core/src/modules/order/api/mod.rs`
  - `apps/platform-core/src/modules/order/dto/mod.rs`
...
- 验证步骤：
  ...
  5. 启动服务并联调：
     `APP_PORT=18080 APP_HOST=127.0.0.1 DATABASE_URL=postgres://datab:datab_local_pass@127.0.0.1:5432/datab KAFKA_BROKERS=127.0.0.1:9094 KAFKA_BOOTSTRAP_SERVERS=127.0.0.1:9094 MINIO_ENDPOINT=http://127.0.0.1:9000 OPENSEARCH_ENDPOINT=http://127.0.0.1:9200 cargo run -p platform-core`
  6. `curl -X POST /api/v1/orders/{id}/contract-confirm`（可确认订单）。
  7. `curl -X POST /api/v1/orders/{id}/contract-confirm`（不可确认状态订单）。
  8. `psql` 校验 `trade.order_main`、`contract.digital_contract`、`contract.contract_signer`、`audit.audit_event`。
- 验证结果：
  - `cargo test -p platform-core`：通过（`96 passed, 0 failed, 1 ignored`）。
  - `trade006_contract_confirm_db_smoke`：通过。
  - API 联调通过：
    - 成功分支 `req-trade006-api-1` 返回 `200`，`contract_status=signed`、`order_status=contract_effective`、`signer_role=buyer_operator`。
    - 冲突分支 `req-trade006-api-2` 返回 `409`，`ORDER_CONTRACT_CONFIRM_FORBIDDEN`。
  - DB 证据：
    - `trade.order_main`：`status=contract_effective`，`contract_id` 已关联，`last_reason_code=TRADE-006`。
    - `contract.digital_contract`：`status=signed`，`contract_digest=sha256:trade006-api-1`。
    - `contract.contract_signer`：存在 `signer_role=buyer_operator` 与签署人。
    - `audit.audit_event`：存在 `action_name=trade.contract.confirm`，`request_id=req-trade006-api-1`。
  - 清理：临时业务测试数据已删除；审计表 append-only，测试审计记录保留。
- 覆盖的冻结文档条目：
  - `docs/领域模型/全量领域模型与对象关系说明.md`（4.3 合同与策略聚合）
  - `docs/页面说明书/页面说明书-V1-完整版.md`（6.2 合同确认页）
  - `docs/全集成文档/数据交易平台-全集成基线-V1.md`（15 核心交易链路）
  - `docs/权限设计/接口权限校验清单.md`（3.4 `POST /api/v1/orders/{id}/contract-confirm`）
  - `docs/权限设计/角色权限矩阵正式版.md`（`trade.contract.confirm` 角色口径）
- 覆盖的任务清单条目：`TRADE-006`
- 未覆盖项：无。
- 新增 TODO / 预留项：无新增 `TODO(V1-gap)` / `TODO(V2-reserved)` / `TODO(V3-reserved)`；`TODO-PROC-BIL-001` 追溯约束保持不变。
- 待人工审批结论：待审批
