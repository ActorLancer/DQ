...
| TODO-AUD-OPENAPI-001 | AUD-003~AUD-028 | V1-gap | audit-ops-contract | `packages/openapi/audit.yaml; packages/openapi/ops.yaml; docs/02-openapi/README.md` | accepted | 当前批次仅收缩 `notification.requested / audit.anchor_requested / fabric.proof_submit_requested` 的文档口径与事件拓扑，不提前伪造 `AUD/Ops` OpenAPI 完整实现。 | 进入 `AUD / consistency / integration` 代码实现批次后，补齐 `packages/openapi/audit.yaml`、`packages/openapi/ops.yaml` 与 `docs/02-openapi/audit.yaml`、`docs/02-openapi/ops.yaml` 归档，并同步校验 `event_type / target_topic / aggregate_type` 示例。 | no |
| TODO-AUD-TEST-001 | AUD-009 / AUD-013~017 / NOTIF-001~012 | V1-gap | audit-notif-fabric-cases | `docs/05-test-cases/README.md; docs/04-runbooks/notification-worker.md; docs/04-runbooks/fabric-local.md` | accepted | 当前批次只修正文档设计冲突，不提前补齐通知/Fabric/一致性控制面的测试样例与集成 smoke。 | 进入 `NOTIF / AUD / integration` 代码实现批次后，新增 `docs/05-test-cases/audit-consistency-cases.md` 与通知/Fabric 事件链路验收清单，并把 runbook 中的实操步骤、联查入口、人工补发与 retry/DLQ 流程补完整。 | no |
...
| TODO-NOTIF-CONTRACT-001 | NOTIF-014 | V1-gap | notif-contract-test | `docs/开发任务/问题修复任务/A10-NOTIF-通知链路与命名边界缺口.md; docs/开发任务/问题修复任务/A11-测试与Smoke口径误报风险.md; docs/04-runbooks/notification-worker.md; docs/05-test-cases/README.md; docs/05-test-cases/notification-cases.md` | accepted | `NOTIF-013` 已补齐通知控制面 OpenAPI 归档与示例；当前剩余 gap 只在 `NOTIF-014` 的通知验收清单文件尚未落盘。 | 进入 `NOTIF-014` 后，必须补齐 `docs/05-test-cases/notification-cases.md`，覆盖 `mock-log`、幂等、重试、DLQ、人工补发与审计联查，并验证其与 `notification-worker` 正式链路一致。 | no |
