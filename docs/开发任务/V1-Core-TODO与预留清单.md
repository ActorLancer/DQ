...
| TODO-SEARCHREC-CLEANUP-001 | SEARCHREC-013 | tech-debt | recommendation-audit-cleanup | `apps/platform-core/src/modules/recommendation/tests/recommendation_api_db.rs; docs/开发任务/V1-Core-实施进度日志-P6.md` | accepted | `SEARCHREC-013` 的手工 local smoke 证明：一旦推荐读取请求写入 `audit.access_audit`，删除临时 `core.user_account / core.organization` 会触发外键 `ON DELETE SET NULL`，又被 `audit.access_audit` 的 append-only guard 拦截；当前自动化清理采用“忽略删除失败”的 best-effort 口径，手工联调只能完整清掉 `recommendation_request / product / asset` 等业务对象，无法回收测试 principal。 | 后续需要提供与 append-only 审计兼容的 principal 清理策略，例如复用固定联调 principal、显式 tombstone/inactive 策略，或把审计外键改成不触发回写的冻结引用模型，再同步修正文档与 smoke 清理步骤。 | no |
