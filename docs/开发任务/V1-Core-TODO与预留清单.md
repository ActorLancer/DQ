...
| TODO-ENV-NAMING-001 | ENV-058 | V1-gap | env-naming | `docs/开发任务/问题修复任务/A12-配置项与资源命名漂移.md; docs/04-runbooks/kafka-topics.md; docs/04-runbooks/local-startup.md` | accepted | 当前批次只把配置项、bucket、alias、Redis key 与 compose 主入口的统一命名义务写入执行源，不提前修改全部运行时和脚本。 | 进入 `ENV / SEARCHREC / AUD` 相关实现批次后，必须统一配置清单、初始化脚本、运行时代码和 runbook 的命名，并补最小一致性校验。 | no |
