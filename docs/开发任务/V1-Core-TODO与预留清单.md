...
| TODO-AUD-OPENAPI-001 | AUD-003~AUD-028 | V1-gap | audit-ops-contract | `packages/openapi/audit.yaml; packages/openapi/ops.yaml; docs/02-openapi/README.md` | accepted | `AUD-003` 已补齐 `GET /api/v1/audit/orders/{id}`、`GET /api/v1/audit/traces`，`AUD-004` 已补齐 `POST /api/v1/audit/packages/export` 及对应 `docs/02-openapi/audit.yaml` 归档；但后续高风险控制面与 `ops.yaml` 仍未完成。 | 继续在 `AUD-005+` 批次补齐 legal hold、replay、dead letter reprocess、一致性修复等高风险接口与 `packages/openapi/ops.yaml` / `docs/02-openapi/ops.yaml`，并同步校验 `event_type / target_topic / aggregate_type` 示例。 | no |
