...
| TODO-AUD-OPENAPI-001 | AUD-003~AUD-028 | V1-gap | audit-ops-contract | `packages/openapi/audit.yaml; packages/openapi/ops.yaml; docs/02-openapi/README.md` | accepted | `AUD-003` 已补齐 `GET /api/v1/audit/orders/{id}`、`GET /api/v1/audit/traces`，`AUD-004` 已补齐 `POST /api/v1/audit/packages/export`，`AUD-005` 已补齐 `POST /api/v1/audit/replay-jobs`、`GET /api/v1/audit/replay-jobs/{id}` 及对应 `docs/02-openapi/audit.yaml` 归档；但后续高风险控制面与 `ops.yaml` 仍未完成。 | 继续在 `AUD-006+` 批次补齐 legal hold、anchor / Fabric、dead letter reprocess、一致性修复等高风险接口与 `packages/openapi/ops.yaml` / `docs/02-openapi/ops.yaml`，并同步校验 `event_type / target_topic / aggregate_type` 示例。 | no |
| TODO-AUD-TEST-001 | AUD-009 / AUD-013~017 / NOTIF-001~012 | V1-gap | audit-notif-fabric-cases | `docs/05-test-cases/README.md; docs/05-test-cases/audit-consistency-cases.md; docs/04-runbooks/notification-worker.md; docs/04-runbooks/fabric-local.md; docs/04-runbooks/audit-replay.md` | accepted | `AUD-005` 已补齐 audit query / export / replay 的首版验收矩阵与 replay runbook，但通知/Fabric/一致性控制面的完整测试样例与集成 smoke 仍未全部落盘。 | 进入 `AUD-006+ / integration / Fabric / consistency` 代码实现批次后，继续在 `docs/05-test-cases/audit-consistency-cases.md` 追加 legal hold、anchor、callback、reconcile、dead letter reprocess 清单，并把相关 runbook 的实操步骤、联查入口和补救流程补完整。 | no |
