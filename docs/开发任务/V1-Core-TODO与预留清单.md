...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TODO-001 | TASK-ID | V1-gap / V2-reserved / V3-reserved / tech-debt | module-name | path/to/file | status-here | 简述原因 | 简述补齐条件 | yes / no |
...
| TODO-PROC-BIL-001 | BIL-* | tech-debt | process-governance | `docs/开发任务/V1-Core-实施进度日志.md; docs/开发任务/V1-Core-实施进度日志-P1.md; docs/开发任务/V1-Core-实施进度日志-P3.md` | closed | 已在 `BIL` 阶段完成追溯性一致性复核：`BIL-001~005` 在 `P3` 逐条按冻结文档重新核对并补齐，`TRADE-030` 也在 `P1` 记录其依赖 `BIL-005` 后按正式口径实现支付结果编排；期间暴露的返工项（如 `BIL-002/BIL-003` 审计补齐、`BIL-003` 状态历史一致性修复）均已落盘并通过后续阶段链路验证，不再保留“历史偏移但未复核”的治理缺口。 | 无；后续若再次出现跨阶段偏移，应新开独立 process-governance TODO，而不是复用本条。 | no |
...
| TODO-AUD-OPENAPI-001 | AUD-003~AUD-028 | V1-gap | audit-ops-contract | `packages/openapi/audit.yaml; packages/openapi/ops.yaml; packages/openapi/search.yaml; docs/02-openapi/audit.yaml; docs/02-openapi/ops.yaml; docs/02-openapi/search.yaml; docs/02-openapi/README.md` | closed | 已在 `AUD-003~024` 补齐审计 / ops / search 运维控制面契约，并在 `AUD-027` 重新收口 `docs/02-openapi/{audit,ops}.yaml` 与 `packages/openapi` 的逐字同步、README 索引与 `./scripts/check-openapi-schema.sh` 校验；`AUD-025+` 的剩余高风险控制面也已全部落盘，不再存在“实现已完成但 OpenAPI 仍待补”的缺口。 | 无；后续仅在新增 `AUD / SEARCHREC` 正式接口时继续按同一校验脚本与归档副本规则扩展。 | no |
| TODO-AUD-TEST-001 | AUD-013~028 | V1-gap | audit-notif-fabric-cases | `docs/05-test-cases/README.md; docs/05-test-cases/audit-consistency-cases.md; docs/05-test-cases/canonical-event-authority-cases.md; docs/04-runbooks/fabric-local.md; docs/04-runbooks/fabric-adapter.md; docs/04-runbooks/fabric-event-listener.md; docs/04-runbooks/fabric-ca-admin.md; docs/04-runbooks/audit-replay.md; docs/04-runbooks/audit-legal-hold.md; docs/04-runbooks/audit-anchor-batches.md; docs/04-runbooks/audit-ops-outbox-dead-letters.md; docs/04-runbooks/audit-dead-letter-reprocess.md; docs/04-runbooks/outbox-publisher.md; docs/04-runbooks/audit-consistency-lookup.md; docs/04-runbooks/audit-consistency-reconcile.md; docs/04-runbooks/audit-trade-monitor.md; docs/04-runbooks/audit-external-facts.md; docs/04-runbooks/audit-fairness-incidents.md; docs/04-runbooks/audit-projection-gaps.md; docs/04-runbooks/audit-observability.md; docs/04-runbooks/developer-trace.md; docs/04-runbooks/audit-authority-writer.md; docs/04-runbooks/canonical-event-authority.md; docs/04-runbooks/search-reindex.md` | closed | `AUD-013~024` 的宿主机联调、DB smoke、MinIO / Kafka / 观测栈回查、runbook 与验收矩阵已在对应批次全部落盘；`AUD-028` 已把五类一致性场景显式映射到正式对象；`AUD-030` 补齐 canonical event authority 验收清单，`AUD-031` 补齐 Billing bridge published-only 边界，当前批次又补上 `fabric-adapter` 重复投递隔离 case，测试矩阵已覆盖 `AUD` 阶段剩余正式控制面。 | 无；后续只在新增阶段能力时按同一 README 索引与验收矩阵模式追加。 | no |
| TODO-AUD-FABRIC-001 | AUD-013; AUD-026 | V1-gap | fabric-adapter-reliability | `services/fabric-adapter/internal/service/processor.go; services/fabric-adapter/internal/service/redis_lock.go; services/fabric-adapter/internal/service/processor_live_smoke_test.go; docs/04-runbooks/fabric-adapter.md; docs/05-test-cases/audit-consistency-cases.md` | closed | 已为 Go 版 `fabric-adapter` 接入正式 `ops.consumer_idempotency_record + Redis` 短锁门禁；重复投递会先命中 `datab:v1:fabric-adapter:consumer-lock:{event_id}`，再由 `ops.consumer_idempotency_record` 记录 `processing / processed / failed` 结果，避免重复调用 provider / 重复写回 `ops.external_fact_receipt`。当前批次同时补齐了 `TestProcessorReliabilityLiveSmoke`、runbook 与验收矩阵。 | 无；后续仅在需要扩展更复杂的 consumer 重试策略时按同一 DB+Redis 双层门禁继续演进。 | no |
| TODO-AUD-WRITER-001 | AUD-001; AUD-002; AUD-029 | V1-gap | audit-writer-authority | `docs/开发任务/问题修复任务/A06-Audit-Kit-统一模型漂移.md; docs/开发任务/问题修复任务/A14-AUD-历史模块统一Writer与证据桥接缺口.md; docs/04-runbooks/audit-authority-writer.md` | closed | `AUD-001 / AUD-002` 已收口统一 `AuditEvent / EvidenceItem / EvidenceManifest` authority model，`AUD-029` 又把 `catalog / search / billing dispute` 的正式写入收口到统一 `audit writer / evidence writer`，并通过 `audit-authority-writer.md` 固化最小 smoke 与 `support.evidence_object -> audit.evidence_*` 桥接规则。当前已不存在该 TODO 所描述的阶段内缺口。 | 无；后续仅在扩展到 `order / delivery` 等不在 `AUD-029` 范围内的历史模块时按同一 authority model 继续复核。 | no |
...
| TODO-SEARCHREC-CONSUMER-001 | AUD-008; AUD-010; AUD-026; SEARCHREC-015; SEARCHREC-020 | V1-gap | searchrec-consumer-reliability | `docs/开发任务/问题修复任务/A15-SEARCHREC-Consumer-幂等与DLQ闭环缺口.md; docs/04-runbooks/search-reindex.md; docs/04-runbooks/recommendation-runtime.md; docs/05-test-cases/search-rec-cases.md` | closed | `AUD-026` 已在 `search-indexer / recommendation-aggregator` 运行时代码中落地 `event_id` 幂等、`ops.consumer_idempotency_record`、`ops.dead_letter_event + dtp.dead-letter` 双层隔离、`enable.auto.commit=false` 与“成功或失败已安全隔离后才提交 offset”；`SEARCHREC-020` 已通过 `search_indexer_db_smoke`、`recommendation_aggregator_db_smoke`、`audit_dead_letter_reprocess_db_smoke` 与 runbook/test-case 回查完成正式验收。 | 无；后续仅在 `SEARCHREC-015/017` 综合测试矩阵中继续引用同一 smoke / reprocess 命令，不得回退为只看 outbox 行或手工 seed OpenSearch 的伪验证。 | no |
| TODO-SEARCHREC-AUTH-001 | AUD-022; SEARCHREC-018; SEARCHREC-019; SEARCHREC-015; SEARCHREC-016; SEARCHREC-017 | V1-gap | searchrec-auth-contract | `docs/开发任务/问题修复任务/A13-SEARCHREC-统一鉴权-Step-Up-审计与契约口径缺口.md; docs/04-runbooks/search-reindex.md; docs/04-runbooks/recommendation-runtime.md` | closed | `SEARCHREC-018/019` 已把搜索 / 推荐运行态统一切到正式 Bearer 鉴权门面、正式权限点、必要 `X-Idempotency-Key / X-Step-Up-Token`、真实 `iam.step_up_challenge` 绑定、`audit.audit_event + audit.access_audit + ops.system_log` 与 `SEARCH_*` 错误码；`SEARCHREC-015/016/017` 又通过 smoke、OpenAPI 归档与测试矩阵把该口径固定下来，不再存在运行态和契约侧的遗留缺口。 | 无；后续仅在新增 Search / Recommendation 正式接口时按同一鉴权与审计口径扩展。 | no |
| TODO-SEARCHREC-CONTRACT-TEST-001 | SEARCHREC-015; SEARCHREC-016; SEARCHREC-017 | V1-gap | searchrec-openapi-test | `packages/openapi/search.yaml; packages/openapi/recommendation.yaml; docs/02-openapi/search.yaml; docs/02-openapi/recommendation.yaml; docs/05-test-cases/search-rec-cases.md` | closed | `SEARCHREC-016` 已把 Search / Recommendation OpenAPI 提升为“源码版 + 归档版逐字同步”，并由 `./scripts/check-openapi-schema.sh` 强制校验 Bearer、权限点、幂等、必要 `step-up`、审计与错误码；`SEARCHREC-017` 已把验收矩阵映射到真实 smoke / DB / OpenSearch / Redis / Kafka 证据，契约和测试不再停留在待补齐状态。 | 无；后续仅在新增 SEARCHREC 正式接口时继续扩展同一 OpenAPI/测试矩阵校验。 | no |
| TODO-SEARCHREC-FALLBACK-001 | SEARCHREC-001; SEARCHREC-004 | V1-gap | searchrec-runtime-mode | `docs/开发任务/问题修复任务/A07-搜索同步链路与搜索接口闭环缺口.md; docs/04-runbooks/local-startup.md` | closed | 已在 `BATCH-247` 把搜索运行边界收口到当前代码与 runbook：`staging` 强制 `OpenSearch` 候选源与启动自检，`local / demo` 允许 `PostgreSQL` 搜索投影 fallback，并通过真实 DB smoke 验证 `backend=opensearch / postgresql`、Redis 搜索缓存与最终 PostgreSQL 放行校验。 | 无；后续 `SEARCHREC-004+` 继续扩展搜索查询能力时，必须保持同一运行边界，不得把 fallback 重新降级成口头目标。 | no |
| TODO-SEARCHREC-CLEANUP-001 | SEARCHREC-013 | tech-debt | recommendation-audit-cleanup | `apps/platform-core/src/modules/recommendation/tests/recommendation_api_db.rs; docs/05-test-cases/search-rec-cases.md; docs/开发任务/V1-Core-实施进度日志-P6.md` | closed | 已在推荐 DB smoke 中把 principal 清理策略从“删除失败后忽略”收口为正式 tombstone：业务对象与派生表仍会真实删除，产生审计引用的 `core.user_account / core.organization` 则改为 `status='inactive'` 且写入 `cleanup_state='tombstoned'`，避免再触发 `audit.access_audit` append-only 外键回写冲突；相关策略已同步写入 SEARCHREC 测试说明。 | 无；后续若其他阶段的联调 principal 也受 append-only 审计约束，复用同一 tombstone 策略即可。 | no |
| TODO-AUD-EVENT-001 | AUD-030; AUD-031 | V1-gap | audit-event-authority | `docs/开发任务/问题修复任务/A02-统一事件-Envelope-与路由权威源.md; docs/开发任务/问题修复任务/A05-Outbox-Publisher-DLQ-统一闭环缺口.md; docs/04-runbooks/kafka-topics.md; docs/04-runbooks/canonical-event-authority.md; docs/05-test-cases/canonical-event-authority-cases.md` | closed | `AUD-030` 已把 canonical envelope、`ops.event_route_policy` 唯一路由权威、`common.tg_write_outbox()` 退役与无双写 smoke 显式冻结；`AUD-031` 已进一步清理把 `ops.outbox_event` 当私有工作队列的旁路，只保留 `outbox publisher -> Kafka -> consumer -> dead letter / reprocess` 正式闭环，并把 Billing bridge 收紧为 published-only。 | 无；后续仅在新增 topic / route policy 时按同一 canonical authority 与验收矩阵扩展。 | no |
...
| TODO-SEARCHREC-ALIAS-001 | SEARCHREC-021 | V1-gap | search-alias-authority | `docs/开发任务/问题修复任务/A08-搜索Alias权威源与阶段边界冲突.md; docs/04-runbooks/search-reindex.md` | closed | `SEARCHREC-021` 已统一 `search.index_alias_binding`、运行时默认值、初始化脚本、ops 接口与 runbook 对 `product_search_read/write`、`seller_search_read/write` 的唯一 authority；`alias switch` 也已通过真实 OpenSearch / PostgreSQL 回查固定为 `V1` 最小运维能力，不再存在“只写任务清单但未落实现”的残留。 | 无；后续若扩展灰度切换或自动回滚，仅作为更高阶段能力继续演进。 | no |
...
| TODO-TEST-CANONICAL-001 | TEST-028 | V1-gap | test-smoke-authority | `scripts/check-canonical-contracts.sh; docs/开发任务/问题修复任务/A01-Kafka-Topic-口径统一.md; docs/开发任务/问题修复任务/A11-测试与Smoke口径误报风险.md; docs/05-test-cases/README.md; .github/workflows/canonical-contracts.yml` | closed | 已补齐 `scripts/check-canonical-contracts.sh` 正式 checker：本地 `full` 模式会串行执行 `check-openapi-schema.sh + check-topic-topology.sh + smoke-local.sh`，并追加 canonical consumer group、宿主机/容器 Kafka 边界以及正式运行态文档不再保留旧 topic / 旧命名默认值的静态拦截；CI 侧则通过 `canonical-contracts.yml` 运行 `CANONICAL_CHECK_MODE=static` 静态子集。 | 无；后续仅在新增正式 topic / consumer group / OpenAPI 控制面时继续扩展该 checker 规则。 | no |
...
- `BATCH-288`（`WEB-014`）：无新增 `V1-gap / V2-reserved / V3-reserved / tech-debt` 项；审计联查页与证据包导出页已真实接入 `GET /api/v1/audit/traces`、`GET /api/v1/audit/orders/{id}`、`GET /api/v1/developer/trace`、`GET /api/v1/ops/trade-monitor/orders/{orderId}`、`GET /api/v1/ops/external-facts`、`GET /api/v1/ops/projection-gaps` 与 `POST /api/v1/audit/packages/export`，覆盖五类联查键、链回执、外部事实、投影差异、证据对象隐藏、导出幂等键、step-up、权限/空态/错态/加载态、桌面/移动浏览器 smoke、真实 API、数据库回查与前端不直连受限系统验证；证据包导出产生的审计 artifact 按 append-only 证据链保留。
- `BATCH-289`（`WEB-015`）：无新增 `V1-gap / V2-reserved / V3-reserved / tech-debt` 项；ops 工作台已真实接入一致性联查、outbox、dead letter、观测总览、搜索同步、搜索排序、搜索 reindex/cache、推荐位、推荐排序和推荐重建接口，覆盖幂等键、step-up、权限/空态/错态/加载态、桌面/移动浏览器 smoke、真实 API、数据库/审计/system_log 回查、临时 search reindex 任务清理与前端不直连受限系统验证。
...
- `BATCH-246`（`AUD` 阶段复核与 TODO 收口）：重新复核 `AUD-001~031` 后，关闭 `TODO-AUD-OPENAPI-001`、`TODO-AUD-TEST-001`、`TODO-AUD-FABRIC-001`、`TODO-AUD-WRITER-001`、`TODO-AUD-EVENT-001`、`TODO-TEST-CANONICAL-001` 与 `TODO-PROC-BIL-001`；其中 `fabric-adapter` 已接入 `ops.consumer_idempotency_record + Redis` 短锁、补齐 `TestProcessorReliabilityLiveSmoke`、runbook 与 `AUD-CASE-020A`，并新增 `scripts/check-canonical-contracts.sh + .github/workflows/canonical-contracts.yml` 作为 `TEST-028` 的本地 / CI checker 入口；`TODO-PROC-BIL-001` 则依据 `P3/P1` 中对 `BIL-001~005` 与 `TRADE-030` 的追溯性一致性复核正式关闭。其余四条已由 `AUD-027~031` 的正式交付完成收口。
- `BATCH-247`（`SEARCHREC-001`）：关闭 `TODO-SEARCHREC-FALLBACK-001`；`platform-core` 已按运行 mode 对搜索候选源与启动自检做正式分流，`staging` 继续强制 `OpenSearch`，`local / demo` 可走 `PostgreSQL` 搜索投影 fallback，并通过 `search_api_and_ops_db_smoke`、`search_catalog_pg_fallback_db_smoke`、`search_indexer_db_smoke`、`cargo sqlx prepare --workspace` 与 `./scripts/check-query-compile.sh` 完成验证。当前批次无新增 `V1-gap / V2-reserved / V3-reserved / tech-debt` 项。
...
- `BATCH-259`（`SEARCHREC-013`）：新增非阻塞技术债 `TODO-SEARCHREC-CLEANUP-001`；`APP_MODE=local` 下的推荐读取已补齐 PostgreSQL 搜索投影驱动的最小候选召回、缓存隔离与零结果兜底，并通过 staging/local 双 smoke、全量测试、`sqlx prepare`、query compile check 和手工 `curl + psql` 联调验证；手工清理阶段发现 append-only 审计引用会阻断测试 principal 的物理删除，因此登记后续 principal cleanup 策略补齐义务。
- `BATCH-268`（`SEARCHREC` 残留 TODO 收口复核）：关闭 `TODO-SEARCHREC-AUTH-001`、`TODO-SEARCHREC-CONTRACT-TEST-001`、`TODO-SEARCHREC-ALIAS-001` 与 `TODO-SEARCHREC-CLEANUP-001`；前 3 项已由 `SEARCHREC-018/019/015/016/017/021` 的实现、OpenAPI、test-case 与 runbook 证据实际完成，本批补上台账收口；最后 1 项则通过推荐 smoke 的正式 tombstone principal 清理策略落地并验证关闭。
...
- `BATCH-263`（`SEARCHREC-020`，单任务批次）：无新增 `V1-gap / V2-reserved / V3-reserved` 项；已以 `search_indexer_db_smoke`、`recommendation_aggregator_db_smoke`、`audit_dead_letter_reprocess_db_smoke` 和 runbook/test-case 回查正式验收 SEARCHREC consumer 可靠性闭环，并关闭 `TODO-SEARCHREC-CONSUMER-001`；当前仅保留 `SEARCHREC-015/016/017` 的综合测试矩阵与契约归档任务，后续不得再把“只看 outbox 行/手工 seed”当作 worker 可靠性验证。
- `BATCH-277`（`WEB-003`，单任务批次）：无新增 `V1-gap / V2-reserved / V3-reserved / tech-debt` 项；门户首页已真实接入 `catalog/standard-scenarios`、`recommendations(home_featured)`、`catalog/search` 与 `ops.healthReady`，并补齐 Bearer claims 会话、官方五链路冻结回退、桌面/移动浏览器 smoke、数据库回查与 `sdk-ts` 绝对 URL 查询拼接回归测试；本批未引入新的 TODO，后续按 `WEB-004+` 继续扩展搜索页、商品详情页和卖方主页的正式业务细节。
- `BATCH-278`（`WEB-004`，单任务批次）：无新增 `V1-gap / V2-reserved / V3-reserved / tech-debt` 项；商品搜索页已真实接入 `GET /api/v1/catalog/search` 与 `packages/sdk-ts` 生成契约，覆盖关键词、当前契约内筛选、排序、分页、结果卡片、权限态、加载态、空态、错态、浏览器 Bearer smoke 与审计回查；当前未实现 Facet 聚合、卖方主体类型、敏感等级、价格模式、供方筛选是因为 OpenAPI / 后端 SearchQuery 尚未提供对应字段，本批未以前端 TODO 或 mock 伪造。
- `BATCH-284`（`WEB-010`，单任务批次）：无新增 `V1-gap / V2-reserved / V3-reserved / tech-debt` 项；交付中心页面已接入文件交付、API 开通、共享开通、版本订阅、模板授权、沙箱开通和报告交付入口，写操作统一透传 `X-Idempotency-Key`，真实 `FILE_SUB` 订阅 API、审计 metadata、数据库回查、清理业务测试数据、桌面/移动浏览器 smoke 与前端受控 API 边界校验均通过。
- `BATCH-285`（`WEB-011`，单任务批次）：无新增 `V1-gap / V2-reserved / V3-reserved / tech-debt` 项；验收页面已接入 `auth/me`、订单详情、生命周期快照、验收通过与拒收接口，写操作统一透传 `X-Idempotency-Key`，拒收原因、生命周期摘要、审计提示、权限态、空态、错态、桌面/移动浏览器 smoke、真实 API、数据库回查与前端受控 API 边界校验均通过。
- `BATCH-290`（`WEB-016`，单任务批次）：无新增 `V1-gap / V2-reserved / V3-reserved / tech-debt` 项；开发者页面已在 portal / console 同步接入应用管理、API Key 轮换 / 撤销、developer trace 联查与 Mock 支付操作入口，写操作统一透传 `Idempotency-Key`，敏感页面显示主体 / 角色 / 租户 / 作用域，真实 API、DB 回查、浏览器 smoke、E2E、OpenAPI 防漂移、SDK 契约和受控 API 边界校验均通过。
