...
| TODO-NOTIF-CONTRACT-001 | NOTIF-013; NOTIF-014 | V1-gap | notif-contract-test | `docs/开发任务/问题修复任务/A10-NOTIF-通知链路与命名边界缺口.md; docs/开发任务/问题修复任务/A11-测试与Smoke口径误报风险.md; docs/04-runbooks/notification-worker.md; docs/02-openapi/README.md; docs/05-test-cases/README.md` | accepted | 当前批次只把 NOTIF 的控制面 OpenAPI 与通知验收清单义务写入执行源，不提前伪造通知发送记录模型、OpenAPI 完成态或测试文件。 | 进入 `NOTIF` 代码实现批次后，必须补齐通知联查/人工补发的 OpenAPI 示例与 `docs/05-test-cases/notification-cases.md`，并验证其与 `notification-worker` 正式链路一致。 | no |
