...
- 本文当前角色：统一事务模板、真实审计写入与 outbox writer 的治理说明，用于约束后续 `AUD` 收口顺序与遗留迁移边界。
- 本文第 `2` 节与第 `4` 节保留问题发现时的历史起点；当前实际运行状态应同时以任务清单、运行时代码、runbook 与实施日志为准。
- 后续如统一事务模板、真实 writer 或遗留模块迁移状态发生变化，必须先更新本文件中的当前状态与剩余未完成项。
