...
- 本文当前角色：`NOTIF` 阶段命名、链路边界与后续实现义务的治理说明。
- 当前通知拓扑、正式进程命名、`V1` 渠道边界已经冻结；但通知运行时、发送记录、联查接口、OpenAPI、测试与 runbook 仍主要由后续 `NOTIF` 实现批次承接。
- 第 `2` 节与第 `4` 节保留问题发现时的历史起点，不应被误读为“当前仍无任何冻结口径”。
