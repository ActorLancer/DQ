...
- 本文当前角色：搜索 alias authority、命名与阶段边界的治理说明，用于约束后续运维接口、初始化脚本和测试继续沿同一口径推进。
- 当前首轮命名与 authority 收口结果见第 `9.4` 节；第 `2` 节与第 `4` 节保留问题发现时的历史起点。
- 后续若继续扩展 alias switch、灰度切换或自动回滚策略，必须先在任务清单和 runbook 中冻结阶段边界，再回写本文。
