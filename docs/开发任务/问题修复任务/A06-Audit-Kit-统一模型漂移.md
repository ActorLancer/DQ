...
- 本文当前角色：`audit-kit` 统一模型、正式审计对象与后续 `AUD` 承接边界的治理说明。
- 当前 `audit-kit` 的统一模型仍是后续 `AUD-001 / AUD-002 / AUD-029` 的核心前置；本文第 `2` 节与第 `4` 节保留问题发现时的历史起点。
- 若后续 `audit-kit`、DB schema 或 OpenAPI/DTO 映射发生变化，必须同步更新本文中的当前状态与剩余未完成项。
