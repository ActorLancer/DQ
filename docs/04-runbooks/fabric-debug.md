...
- `AUD-015` 起，还要回查 `ops.external_fact_receipt.metadata.callback_event_id / provider_request_id / provider_status / payload_hash` 是否齐备，并确认 `dtp.fabric.callbacks` 中存在对应 `fabric.commit_confirmed / fabric.commit_failed` 事件。
