...
当前文件承接 `AUD-003`、`AUD-004`、`AUD-005`、`AUD-006`、`AUD-007`、`AUD-008`、`AUD-009`、`AUD-010`、`AUD-011`、`AUD-012`、`AUD-013`、`AUD-014`、`AUD-015`、`AUD-016`、`AUD-017`、`AUD-018`、`AUD-019`、`AUD-020`、`AUD-021`、`AUD-023`、`AUD-024`、`AUD-026`、`AUD-031` 已落地的首版审计控制面验收矩阵，并在 `AUD-028` 中把“链下成功链上失败、链上成功链下未更新、回调乱序 / 晚到、重复事件、修复演练”五类一致性场景收口到正式用例与手工 drill，覆盖：
...
| 重复事件 | `V1` 分两层验收：同一 source receipt 一旦写入 `listener_callback_event_id`，`fabric-event-listener` 不得再次补发 callback；consumer 侧重复投递或重试必须通过 `ops.consumer_idempotency_record` 与 `ops.dead_letter_event` 可观测，不能靠普通日志口头去重 | `AUD-CASE-014`、`AUD-CASE-017`、`AUD-CASE-020A`、`AUD-CASE-021`；`docs/04-runbooks/fabric-adapter.md`、`docs/04-runbooks/fabric-event-listener.md`、`docs/04-runbooks/audit-dead-letter-reprocess.md`、`docs/04-runbooks/audit-ops-outbox-dead-letters.md` | 复跑同一 source receipt 的 listener 轮询，执行 `audit_dead_letter_reprocess_db_smoke`，或运行 `cd services/fabric-adapter && FABRIC_ADAPTER_RELIABILITY_SMOKE=1 go test ./internal/service -run TestProcessorReliabilityLiveSmoke -count=1 -v` | `ops.external_fact_receipt.metadata.listener_callback_event_id` 已存在且 callback receipt 数量不增长；`ops.consumer_idempotency_record(result_code='processed' / 'dead_lettered' / 等价去重状态)`、`ops.dead_letter_event`、`audit.audit_event` |
...
| `AUD-CASE-016A` | Billing bridge 只处理已命中 publish attempt 的 bridge outbox | 先运行 `cargo test -p outbox-publisher outbox_publisher_db_smoke -- --nocapture` 或在 `bil024` smoke 中为 `billing.trigger.bridge` 写入 `ops.outbox_publish_attempt(result_code='published')`；再调用 `POST /api/v1/billing/{order_id}/bridge-events/process` | 仅当 `billing.trigger.bridge` 同时满足 `ops.outbox_event.status='published'`、`published_at IS NOT NULL` 且最新 `ops.outbox_publish_attempt.result_code='published'` 时才允许物化 BillingEvent；`pending` 或缺少 publish attempt 的 bridge outbox 不得被处理；物化后的 `billing.billing_event.metadata` 必须保留 `bridge_outbox_event_id / bridge_publish_attempt_id` | API 响应、`ops.outbox_publish_attempt`、`billing.billing_event.metadata`、`docs/04-runbooks/outbox-publisher.md`、`apps/platform-core/src/modules/billing/tests/bil024_billing_trigger_bridge_db.rs` |
...
| `AUD-CASE-020A` | Fabric adapter 重复投递隔离 | `cd services/fabric-adapter && FABRIC_ADAPTER_RELIABILITY_SMOKE=1 DATABASE_URL=postgres://datab:datab_local_pass@127.0.0.1:5432/datab REDIS_URL=redis://:datab_redis_pass@127.0.0.1:6379/4 go test ./internal/service -run TestProcessorReliabilityLiveSmoke -count=1 -v` | 同一 `event_id` 的并发重复调用只允许首条真正执行 provider / receipt write-back；第二条必须被 `Redis` 短锁 + `ops.consumer_idempotency_record` 吸收，不再重复写 `ops.external_fact_receipt` | `ops.consumer_idempotency_record(result_code='processed', metadata.lock_backend='redis', metadata.lock_key~'fabric-adapter:consumer-lock')`、`ops.external_fact_receipt(count=1)`、Redis 短锁存在与释放 |
