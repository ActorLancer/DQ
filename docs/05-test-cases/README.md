...
- `./scripts/check-api-contract-baseline.sh` 是 `TEST-003` 的正式 checker：它校验 API/OpenAPI 相关冻结契约（成功/失败 envelope、关键响应字段、错误码基线、订单状态机 action enum / 禁止错误码绑定），并额外校验运行时代码与 message 口径一致性（拒绝通用 fallback code + 业务前缀 message 残留）；它不替代 `TEST-028` 的 canonical smoke。
...
- `qry-lite-e2e.md`：`TEST-026` 的正式 `QRY_LITE` 端到端验收清单，冻结 seller grant、buyer query run/read、risk refund、`trade013 / dlv011 / dlv012 / dlv013 / bil024` 后端证据与 `audit / outbox / MinIO / billing` 汇总边界。
