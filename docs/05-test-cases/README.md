...
- `audit-consistency-cases.md`：已落地 `AUD-003~AUD-013` 的审计联查、证据包导出、replay dry-run、legal hold、anchor batch、canonical outbox / dead letter 查询、outbox publisher、SEARCHREC dead letter dry-run 重处理、一致性联查、一致性修复 dry-run 与 `fabric-adapter` 基础框架验收矩阵；后续 `Fabric callback / CA / test-network` 继续在同文件追加。
