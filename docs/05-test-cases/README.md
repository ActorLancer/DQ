...
- `audit-consistency-cases.md`：已落地 `AUD-003~AUD-022` 的审计联查、证据包导出、replay dry-run、legal hold、anchor batch、canonical outbox / dead letter 查询、outbox publisher、SEARCHREC dead letter dry-run 重处理、一致性联查、一致性修复 dry-run、`fabric-adapter` 四类摘要 handler、`fabric-event-listener` callback、`fabric-ca-admin` 证书治理、trade monitor 总览 / checkpoints、external facts 查询 / confirm、公平性事件查询 / handle、projection gaps 查询 / resolve，以及搜索运维控制面验收矩阵；后续 `AUD-023+` 的剩余高风险控制面继续在同文件追加。
