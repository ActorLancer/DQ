...
| `recommendation-service` | `platform-core::recommend`（策略与放行） + 外围 worker（离线计算） | 推荐结果最终放行在 `platform-core` | 行为流与离线计算异步处理 | `platform-core` + worker |
| `notification-service` | `services/notification-service`（独立进程） | 无主交易同步写边界 | 消费 outbox/Kafka 发送站内信、邮件、Webhook | `notification-service` |
