...
- `ops.yaml`：当前已归档健康检查、内部开发端点、`NOTIF-013` 的通知模板预览 / 手工注入 / 审计联查 / dead letter replay 契约，以及 `AUD-008` 的 canonical outbox / dead letter 查询、`AUD-011/012` 的一致性联查 / dry-run reconcile、`AUD-018` 的 trade monitor 总览 / checkpoints、`AUD-019` 的 external facts 查询 / confirm、`AUD-020` 的 fairness incidents 查询 / handle、`AUD-021` 的 projection gaps 查询 / resolve；后续进入 `AUD-022+` 的搜索运维控制面继续补齐。
