...
- 种子脚本：`db/seeds/001_base_lookup.sql`、`db/seeds/010_test_tenants.sql`、`db/seeds/020_test_products.sql`、`db/seeds/030_test_orders.sql`、`db/seeds/031_sku_trigger_matrix.sql`、`db/seeds/032_five_scenarios.sql`、`db/seeds/033_searchrec_recommendation_samples.sql`
...
- `db/scripts/verify-migration-070.sh`：验证 `070/072/073/074` 的角色权限最终种子、canonical route-policy 与 retired trigger / function 状态基线。
...
- `db/scripts/verify-seed-033.sh`：验证 `033` 的五条标准链路首页固定推荐样例、推荐位元数据和搜索投影样例基线。
