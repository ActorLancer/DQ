...
const keycloakBaseUrl =
  process.env.WEB_E2E_KEYCLOAK_URL ?? "http://127.0.0.1:8081";
...
const portalUsername =
  process.env.WEB_E2E_PORTAL_USERNAME ?? "local-platform-admin";
const portalPassword =
  process.env.WEB_E2E_PORTAL_PASSWORD ?? "LocalPlatformAdmin123!";
...
test.describe("WEB-018 live portal chain", () => {
  test.skip(
    !liveEnabled,
    "Set WEB_E2E_LIVE=1 to run platform-core backed E2E.",
  );
  ...
  test("logs in with Keycloak and exercises search, product, order, delivery, acceptance and linkage", async ({
    ...
    await expect(page.getByText("角色 platform_admin")).toBeVisible({
      timeout: 20_000,
    });
    await expect(
    ...
    await expect(
      page.getByText("工业设备运行指标 API 订阅").first(),
    ).toBeVisible({
      timeout: 20_000,
    });
    ...
    const orderPayload = await createOrderThroughPortalProxy(
      page,
      idempotencyKey,
    );
...
async function createOrderThroughPortalProxy(
  page: Page,
...
function readCreatedOrderId(payload: unknown): string | undefined {
  ...
  const data =
    root.data && typeof root.data === "object"
      ...
      : undefined;
  const orderId = data?.order_id ?? root.order_id;
