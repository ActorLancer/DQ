...
mod tests {
    ...
    #[tokio::test]
    async fn dlv015_sandbox_model_db_smoke() {
        ...
        let create_request_id = format!("req-dlv015-create-{suffix}");
                ...
                Request::builder()
                    ...
                    .body(Body::from(
                        json!({
                            ...
                            "workspace_name": format!("dlv015-workspace-{suffix}"),
        ...
        let invalid_request_id = format!("req-dlv015-invalid-{suffix}");
    ...
    async fn seed_graph(client: &Client, suffix: &str) -> Result<SeedGraph, db::Error> {
        ...
        let seller_org_id: String = client
            .query_one(
                r#"INSERT INTO core.organization (org_name, org_type, status, metadata)
                 VALUES ($1, 'enterprise', 'active', '{"risk_status":"normal"}'::jsonb)
                 RETURNING org_id::text"#,
                &[&format!("dlv015-seller-{suffix}")],
            )
            .await?
            .get(0);

        let buyer_login_id = format!("dlv015-buyer-user-{suffix}");
        let buyer_display_name = format!("DLV015 Buyer {suffix}");
        ...
        let product_id: String = client
            .query_one(
                ...
                &[
                    ...
                    &asset_version_id,
                    &seller_org_id,
                    &format!("dlv015-product-{suffix}"),
                    &format!("dlv015 product {suffix}"),
                    &format!("dlv015 search {suffix}"),
                ],
            )
            .await?
            .get(0);
        ...
        let sku_id: String = client
            .query_one(
                ...
                &[&product_id, &format!("DLV015-SKU-{suffix}")],
            )
            .await?
            .get(0);
        ...
        let connector_id: String = client
            .query_one(
                ...
                &[
                    &seller_org_id,
                    &format!("dlv015-connector-{suffix}"),
        ...
        client
            .execute(
                ...
                &[&order_id, &format!("sha256:dlv015:{suffix}")],
            )
            .await?;
