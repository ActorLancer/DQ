...
mod tests {
    ...
    #[tokio::test]
    async fn dlv004_download_validation_db_smoke() {
        ...
        let suffix = std::time::SystemTime::now()
            ...
            .as_millis()
            .to_string();
        let object_key = format!("orders/{suffix}/payload.enc");
        let object_body = format!("encrypted-payload-{suffix}").into_bytes();
        let minio_endpoint = spawn_object_server(&object_key, object_body.clone()).await;
        let previous_minio_endpoint = std::env::var("MINIO_ENDPOINT").ok();
        unsafe {
            std::env::set_var("MINIO_ENDPOINT", &minio_endpoint);
            std::env::set_var("MINIO_ROOT_USER", "datab");
            std::env::set_var("MINIO_ROOT_PASSWORD", "datab_local_pass");
        }

        let seed = seed_graph(&client, &suffix).await.expect("seed graph");
        let request_id = format!("req-dlv004-{suffix}");
        let app = crate::with_live_test_state(delivery_router()).await;
        let download_token =
            issue_ticket(&app, &seed.order_id, &seed.buyer_org_id, &request_id).await;
            ...
            .oneshot(
                Request::builder()
                    ...
                    .uri(format!(
                        ...
                        seed.order_id, download_token
                    ))
                    .header("x-role", "buyer_operator")
                    .header("x-tenant-id", &seed.buyer_org_id)
                    .header("x-client-fingerprint", format!("fp-{suffix}"))
        ...
        let decoded = base64::engine::general_purpose::STANDARD
            .decode(object_base64)
            .expect("decode object");
        assert_eq!(decoded, object_body);
        ...
        let ticket_row = client
            ...
            .await
            .expect("ticket row");
        assert_eq!(ticket_row.get::<_, i32>(0), 2);
        assert_eq!(ticket_row.get::<_, String>(1), "exhausted");

        let receipt_row = client
            .query_one(
                "SELECT receipt_hash, client_fingerprint, host(source_ip)
                 FROM delivery.delivery_receipt
            ...
            .await
            .expect("receipt row");
        assert!(!receipt_row.get::<_, String>(0).is_empty());
        assert_eq!(
            receipt_row.get::<_, Option<String>>(1).as_deref(),
            Some(format!("fp-{suffix}").as_str())
        );
        assert_eq!(
            receipt_row.get::<_, Option<String>>(2).as_deref(),
            Some("10.20.30.40")
        );
            ...
            .query_one(
                "SELECT COUNT(*)::bigint
                 ...
                 WHERE action_name = 'delivery.file.download'
                   AND request_id = $1
                   AND ref_type = 'delivery_receipt'
                   AND result_code = 'downloaded'",
                &[&request_id],
            )
            .await
            .expect("audit count")
        ...
        let cached: Option<String> = redis_conn.get(&redis_key).await.expect("redis get");
        assert!(
            cached.is_none(),
            "ticket cache should be deleted when exhausted"
        );
        ...
        let repeat_response = app
            .oneshot(
                Request::builder()
                    ...
                    .header("x-role", "buyer_operator")
                    .header("x-tenant-id", &seed.buyer_org_id)
                    .header("x-request-id", format!("{request_id}-repeat"))
                    .body(Body::empty())
        ...
        if let Some(value) = previous_minio_endpoint {
            unsafe {
                std::env::set_var("MINIO_ENDPOINT", value);
            }
        } else {
            unsafe {
    ...
    async fn issue_ticket(
        ...
        let response = app
            ...
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri(format!("/api/v1/orders/{order_id}/download-ticket"))
                    .header("x-role", "buyer_operator")
                    .header("x-tenant-id", buyer_org_id)
                    .header("x-request-id", request_id)
                    .body(Body::empty())
                    .expect("issue ticket request"),
            )
            .await
            .expect("issue ticket response");
    ...
    async fn spawn_object_server(expected_key: &str, body: Vec<u8>) -> String {
        let app = Router::new()
            .route("/delivery-objects/{*key}", get(serve_object))
            .with_state(ObjectServerState {
        ...
        let addr = listener.local_addr().expect("object server addr");
        tokio::spawn(async move {
    ...
    async fn seed_graph(client: &Client, suffix: &str) -> Result<SeedGraph, db::Error> {
        ...
        let product_id: String = client
            .query_one(
                ...
                &[
                    ...
                    &asset_version_id,
                    &seller_org_id,
                    &format!("dlv004-product-{suffix}"),
                    &format!("dlv004 product {suffix}"),
                    &format!("dlv004 search {suffix}"),
                ],
            )
            .await?
            .get(0);
        ...
        let sku_id: String = client
            .query_one(
                ...
                &[&product_id, &format!("DLV004-SKU-{suffix}")],
            )
            .await?
            .get(0);
        ...
        let storage_namespace_id: String = client
            .query_one(
                ...
                &[&seller_org_id, &format!("dlv004-ns-{suffix}")],
            )
            .await?
            .get(0);
        ...
        let ticket_id: String = client
            .query_one(
                "INSERT INTO delivery.delivery_ticket (
                   ...
                   $1::text::uuid, $2::text::uuid, $3, NOW() + INTERVAL '6 days', 2, 1, 'active'
                 )
