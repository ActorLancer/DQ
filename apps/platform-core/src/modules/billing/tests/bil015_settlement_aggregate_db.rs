...
mod tests {
    ...
    #[tokio::test]
    async fn bil015_settlement_aggregate_db_smoke() {
        ...
        let suffix = std::time::SystemTime::now()
            ...
            .as_millis()
            .to_string();
        let buyer_org_id = seed_org(&client, &format!("bil015-buyer-{suffix}")).await;
        let seller_org_id = seed_org(&client, &format!("bil015-seller-{suffix}")).await;
        let order =
        ...
        record_event(
            ...
            json!({
                ...
                "provider_event_id": format!("bil015-success-{suffix}"),
            }),
        )
        .await;
        record_event(
            ...
            Some("5.00000000"),
            json!({
                "idempotency_key": format!("billing_event:{}:refund", order.order_id),
                "reason_code": "dispute_refund",
                "case_id": format!("00000000-0000-0000-0000-{}", &suffix[suffix.len().saturating_sub(12)..]),
            }),
        )
        .await;
        record_event(
            ...
            Some("3.00000000"),
            json!({
                "idempotency_key": format!("billing_event:{}:comp", order.order_id),
                "reason_code": "service_compensation",
                "case_id": format!("11111111-1111-1111-1111-{}", &suffix[suffix.len().saturating_sub(12)..]),
            }),
        )
        .await;
        record_event(
        ...
        let first = recompute_settlement_for_order(
            ...
            Some(&format!("req-bil015-recompute-1-{suffix}")),
            None,
        )
        .await
        .expect("first recompute");
        let second = recompute_settlement_for_order(
            &client,
            &order.order_id,
            "platform_finance_operator",
            Some(&format!("req-bil015-recompute-2-{suffix}")),
            None,
        )
        .await
        ...
        assert_eq!(first.payable_amount, "88.00000000");
        assert_eq!(first.platform_fee_amount, "2.00000000");
        assert_eq!(first.channel_fee_amount, "1.00000000");
        assert_eq!(first.refund_amount, "5.00000000");
        assert_eq!(first.compensation_amount, "3.00000000");
        assert_eq!(first.net_receivable_amount, "77.00000000");
        assert_eq!(first.settlement_status, "settled");

        let settlement_count: i64 = client
            .query_one(
                "SELECT COUNT(*)::bigint FROM billing.settlement_record WHERE order_id = $1::text::uuid",
                &[&order.order_id],
            )
            .await
        ...
        assert_eq!(
            detail["data"]["settlements"][0]["settlement_status"].as_str(),
            Some("settled")
        );
    ...
    async fn record_event(
        ...
        let _ = record_billing_event(
            ...
            Some(&format!("req-bil015-{event_type}-{order_id}")),
            None,
        )
        .await
        ...
        let response = app
            ...
            .oneshot(
                Request::builder()
                    ...
                    .header("x-request-id", format!("req-bil015-read-{suffix}"))
                    .body(Body::empty())
    ...
    async fn seed_order_without_settlement(
        ...
        let product_id: String = client
            .query_one(
                ...
                &[
                    ...
                    &asset_version_id,
                    &seller_org_id,
                    &format!("bil015-product-{suffix}"),
                    &format!("bil015 product {suffix}"),
                    &format!("bil015 summary {suffix}"),
                ],
            )
            .await
        ...
        let sku_id: String = client
            .query_one(
                ...
                &[&product_id, &format!("BIL015-SKU-{suffix}")],
            )
            .await
            .expect("insert sku")
            .get(0);
        ...
        let _ = client
            .execute(
                "DELETE FROM core.organization
                 WHERE org_id = ANY(ARRAY[$1::text::uuid, $2::text::uuid])",
