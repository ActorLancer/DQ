...
mod tests {
    ...
    #[tokio::test]
    async fn trade013_qry_lite_state_machine_db_smoke() {
            ...
            let response = app
                ...
                .oneshot(
                    Request::builder()
                        .method("POST")
                        .uri(format!(
                            "/api/v1/orders/{}/qry-lite/transition",
                            seed.order_id
                        ))
        ...
        let final_row = client
            ...
            .await
            .expect("query order");
        assert_eq!(final_row.get::<_, String>(0), "closed");
    ...
    async fn seed_order_graph(client: &Client, suffix: &str) -> Result<SeedGraph, db::Error> {
        ...
        let product_id: String = client
            .query_one(
                r#"INSERT INTO catalog.product (
                   ...
                   $5, 'listed', 'one_time', 30.00, 'CNY', 'query_service',
                ...
                &[
                    ...
                    &asset_version_id,
                    &seller_org_id,
                    &format!("trade013-product-{suffix}"),
                    &format!("trade013 product {suffix}"),
                    &format!("trade013 search {suffix}"),
                ],
            )
            .await?
            .get(0);
        ...
        let sku_id: String = client
            .query_one(
                ...
                &[&product_id, &format!("TRADE013-SKU-{suffix}")],
            )
            .await?
            .get(0);
