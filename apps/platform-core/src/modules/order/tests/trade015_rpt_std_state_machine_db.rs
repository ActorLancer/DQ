...
mod tests {
    ...
    #[tokio::test]
    async fn trade015_rpt_std_state_machine_db_smoke() {
        ...
        for action in [
            ...
            let request_id = format!("req-trade015-{suffix}-{action}");
            let response = app
                .clone()
                .oneshot(
                    Request::builder()
                        .method("POST")
                        .uri(format!(
                            "/api/v1/orders/{}/rpt-std/transition",
                            seed.order_id
                        ))
        ...
        let final_row = client
            ...
            .await
            .expect("query order");
        assert_eq!(final_row.get::<_, String>(0), "settled");
        ...
        assert_eq!(final_row.get::<_, String>(4), "settled");
        ...
        let conflict_resp = app
            .oneshot(
                Request::builder()
                    ...
                    .header("x-role", "buyer_operator")
                    .header("x-tenant-id", &seed.buyer_org_id)
                    .header("x-request-id", format!("req-trade015-{suffix}-invalid"))
                    .header("content-type", "application/json")
                    .body(Body::from(r#"{"action":"generate_report"}"#))
                    .expect("request should build"),
            )
            .await
            .expect("response");
        ...
        let msg = json["message"]
            ...
            .unwrap_or_default()
            .to_string();
        assert!(msg.contains("RPT_STD_TRANSITION_FORBIDDEN"));
    ...
    async fn seed_order_graph(client: &Client, suffix: &str) -> Result<SeedGraph, db::Error> {
        ...
        let product_id: String = client
            .query_one(
                ...
                &[
                    ...
                    &asset_version_id,
                    &seller_org_id,
                    &format!("trade015-product-{suffix}"),
                    &format!("trade015 product {suffix}"),
                    &format!("trade015 search {suffix}"),
                ],
            )
            .await?
            .get(0);
        ...
        let sku_id: String = client
            .query_one(
                ...
                &[&product_id, &format!("TRADE015-SKU-{suffix}")],
            )
            .await?
            .get(0);
        ...
        client
            .execute(
                ...
                &[&order_id, &format!("sha256:trade015:{suffix}")],
            )
            .await?;
